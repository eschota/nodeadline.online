...
**С Windows:** локальная выкладка без SSH — [`deploy_local.bat`](../deploy_local.bat) и [`deploy_local.env.example`](../deploy_local.env.example) (master/nginx на этой же машине, например eschota). Удалённый VPS — [`deploy.bat`](../deploy.bat) и [`deploy.env.example`](../deploy.env.example): `git push` → SSH → `git pull` → [`tools/ship.sh`](../tools/ship.sh) → [`tools/restart_master.sh`](../tools/restart_master.sh). Подробности в README.
